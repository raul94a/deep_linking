...
async function sendFiles(req, reply) {
    ...
    for await (const data of datum) {
        ...
        const direction = `./files/${filename}`;
...
async function getPicturesByEvent(req,reply){
    const picturesRepo = new PicturesRepository(pool);

    const pictures = await picturesRepo.findByEventId(eventId)
}
